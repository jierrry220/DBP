...
### 3. 渐进式渲染与静态资源优化
- **渲染数量**: **恢复显示所有 Bot (最多 50 个)**，保证游戏热闹氛围
- **资源优化**: 
  - **移动端**: 普通 Bot 和其他玩家使用 **静态 PNG 图片**，只有当前玩家和杀手保留 GIF 动画
  - **桌面端**: 所有角色保持 GIF 动画
- **渲染策略**:
  - **桌面端**: 每批 2-5 个，间隔 0.5-2秒
  - **移动端**: 每批 1-2 个，间隔 0.8-2.5秒 (平滑加载)
- **效果**: 通过替换大量 GIF 为静态图，彻底解决 Safari 内存占用高的问题，同时保留了完整的玩家数量显示。
